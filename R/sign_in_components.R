...
#' pages UI.  It loads in all the JavaScript dependencies to handle noClocksAuthR sign
...
sign_in_js <- function(ns = function(x) x) {
  ...
  firebase_config <- noclocksauthr$firebase_config
  ...
  if (is.null(firebase_config)) {
    ...
    firebase_deps <- htmltools::tagList(
      ...
      tags$script(src = "noclocksauthr/js/auth_firebase.js?version=5"),
      tags$script(paste0("auth_firebase('", ns(''), "', ", noclocksauthr$cookie_expires, ")"))
    )
  }

  htmltools::tagList(
    shinyFeedback::useShinyFeedback(),
    tags$script(src = "noclocksauthr/js/toast_options.js"),
    tags$script(src = "https://cdn.jsdelivr.net/npm/js-cookie@2/src/js.cookie.min.js"),
    firebase_deps,
    tags$script(src = "noclocksauthr/js/auth_main.js?version=5"),
    tags$script(paste0("auth_main('", ns(''), "', ", noclocksauthr$cookie_expires, ")"))
  )
}
      ...
      if (is.null(hold_jwt$jwt)) {
        ...
        if (!is.null(new_user$message) && identical(new_user$message, "Password reset email sent")) {
          ...
          shinyFeedback::showToast(
            ...
            .options = noclocksauthr_toast_options
          )
          return()
        }
