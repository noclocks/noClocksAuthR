...
#' then be sure to set your Plumber API to allow for cookies.
#' @param api_key Your noClocksAuthR API key
#'
...
auth_filter <- function(method = c("basic", "cookie"), api_key = get_api_key()) {
  ...
  function(req, res) {
    ...
    err_msg <- NULL
    req$noclocksauthr_session <- NULL

    # attempt to find session based on cookie

    noclocksauthr_cookie <- req$cookies$noClocksAuthR
    if ("cookie" %in% method) {

      tryCatch({

        if (is.null(noclocksauthr_cookie)) {
          res$status <- 401L # unauthorized
          stop("noClocksAuthR cookie not provided", call. = FALSE)
        }

        # hash the cookie if sent unhashed
        if (grepl("p0.", noclocksauthr_cookie, fixed = TRUE)) {
          noclocksauthr_cookie <- digest::digest(noclocksauthr_cookie)
        }

        r <- httr::GET(
          url = paste0(noclocksauthr$api_url, "/sessions"),
          query = list(
            hashed_cookie = noclocksauthr_cookie,
    ...
    if ("basic" %in% method) {
      ...
      if (is.null(auth_header)) {
        ...
        tryCatch({
          ...
          if (is.null(noclocksauthr_cookie)) {
            noclocksauthr_cookie <- paste0("api-", uuid::UUIDgenerate())
            noclocksauthr_cookie <- digest::digest(noclocksauthr_cookie)
          } else if (grepl("p0.", noclocksauthr_cookie, fixed = TRUE)) {
            # hash the cookie if sent unhashed
