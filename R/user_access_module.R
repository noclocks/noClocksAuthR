...
#'
#' Shiny module UI for the default user access tab in the \code{noClocksAuthR} Admin Panel.
#'
...
user_access_module_ui <- function(id) {
  ...
  shinydashboard::tabItem(
    ...
    # users table
    tags$script(src = "noclocksauthr/js/user_access_module.js?version=2"),
...
#'
#' Server function for the default Shiny module to control user access in the \code{noClocksAuthR}
#' Admin Panel.
#'
...
user_access_module <- function(input, output, session) {
  ...
  users <- reactive({
    ...
    tryCatch({
      ...
      res <- httr::GET(
        url = paste0(noclocksauthr$api_url, "/last-active-session-time"),
        query = list(
          app_uid = noclocksauthr$app_uid
        ),
        httr::authenticate(
