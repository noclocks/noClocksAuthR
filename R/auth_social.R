...
refresh_jwt_pub_key <- function() {
  ...
  assign("jwt_pub_key", jwt_pub_key_out, envir = noclocksauthr)
  ...
  if (!is.null(cache_controls)) {
    ...
    for (elem in split_equals) {
      ...
      if (length(elem) == 2 && trimws(elem[1]) == "max-age") {
        ...
        assign("jwt_pub_key_expires", as.numeric(Sys.time()) + max_age, envir = noclocksauthr)
        break
      }
...
verify_firebase_token <- function(firebase_token) {
  ...
  decoded_jwt <- NULL
  for (key in noclocksauthr$jwt_pub_key) {
  ...
  if (!(as.numeric(decoded_jwt$exp) + .firebase_token_grace_period > curr_time &&
        ...
        decoded_jwt$aud == noclocksauthr$firebase_config$projectId &&
        decoded_jwt$iss == paste0("https://securetoken.google.com/", noclocksauthr$firebase_config$projectId) &&
...
#' @param hashed_cookie the hashed `noClocksAuthR` cookie.  Used for tracking the user
#' session.  This cookie is inserted into the "noClocksAuthR.sessions" table if the
#' JWT is valid.
#'
#'
...
sign_in_social <- function(
  ...
  if (as.numeric(Sys.time()) + .firebase_token_grace_period > noclocksauthr$jwt_pub_key_expires) {
    refresh_jwt_pub_key()
  }
  ...
  if (!is.null(decoded_jwt)) {
    ...
    if (isFALSE(noclocksauthr$is_invite_required) && identical(nrow(invite), 0L)) {
