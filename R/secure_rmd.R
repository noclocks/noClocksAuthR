  ...
  if (length(inames) > 0) {
    warning(
      paste0(
        ...
        " specified in ",
        xname, " and YAML noClocksAuthR header,",
        " using ", xname
      )
    )
    y <- y[
...
#' the output is secured with \code{noClocksAuthR} authentication.
#'
#' @param rmd_file_path the path the to .Rmd file.
#' @param noclocksauthr_config_args arguments to be passed to \code{\link{noclocksauthr_config}}.
...
#' \dontrun{
#'
#' secure_rmd(system.file("examples/rmds/flexdashboard.Rmd", package = "noClocksAuthR"))
#' secure_rmd(
#'   system.file("examples/rmds/flexdashboard.Rmd", package = "noClocksAuthR"),
#'   noclocksauthr_config_args = list(
...
#'   )
#' )
#' secure_rmd(system.file("examples/rmds/flexdashboard_shiny.Rmd", package = "noClocksAuthR"))
#' secure_rmd(system.file("examples/rmds/html_document.Rmd", package = "noClocksAuthR"))
#' secure_rmd(system.file("examples/rmds/pdf_document.Rmd", package = "noClocksAuthR"))
#' io_file_path <- system.file(
#'   "examples/rmds/ioslides/ioslides_presentation.Rmd",
#'   package = "noClocksAuthR"
#' )
...
secure_rmd <- function(
  ...
  yamlnoclocksauthr <- yaml_header$noClocksAuthR
  yamlnoclocksauthr_config <- yamlnoclocksauthr
  # sign_in_page is the only noClocksAuthR YAML value that is not passed to noclocksauthr_config()
  yamlnoclocksauthr_config$sign_in_page <- NULL

  # noclocksauthr_config_args overrides
  # noclocksauthr_config_args
  # remove any NULL
  noclocksauthr_config_args <-
    overwrite_args(noclocksauthr_config_args,
                   yamlnoclocksauthr_config,
                   xname = "noclocksauthr_config_args")


  if (is.null(noclocksauthr_config_args$api_key)) {
    noclocksauthr_config_args$api_key <- get_api_key()
  }

  # Minimum args needed for an app
  if (is.null(noclocksauthr_config_args$app_name)) {
    stop('noClocksAuthR "app_name" must be provided', call. = FALSE)
  }
  if (is.null(noclocksauthr_config_args$api_key)) {
    stop('noClocksAuthR "api_key" must be provided', call. = FALSE)
  }

  # check that no invalid values passed in via noClocksAuthR YAML values
  if (!all(names(noclocksauthr_config_args) %in% c(
    "app_name",
    "api_key",
    "firebase_config",
    "is_invite_required",
    ...
    stop('Invalid value passed to "noclocksauthr_config_args"', call. = FALSE)
  }
  ...
  hold_sign_in_page <- yamlnoclocksauthr$sign_in_page
  ...
  if (!is.null(hold_sign_in_page)) {
    ...
    if (!all(names(hold_sign_in_page) %in% c("color", "company_name", "logo", "background_image"))) {
      stop("Invalid value passed to noClocksAuthR `sign_in_page` in YAML header.", call. = FALSE)
    }
  ...
  if (!is.null(yaml_header$runtime) &&
    ...
    shiny::addResourcePath("noclocksauthr_static", static_file_dir)
    ...
    embeded_app <- tags$iframe(
      src = file.path("noclocksauthr_static", static_file_name),
      height = "100%",
      width = "100%",
