...
two_fa_ui <- function() {
  ...
  fluidPage(
    ...
    shinyjs::useShinyjs(),
    tags$head(
      tags$link(rel = "shortcut icon", href = "noclocksauthr/images/favicon.ico"),
    ...
    tags$script(src = "noclocksauthr/js/two_fa.js?version=4")
  )
}
  ...
  observeEvent(two_fa_code(), {
    ...
    if (is.na(two_fa_code())) {
      ...
      session$sendCustomMessage(
        ...
        message = list(
          "url" = utils::URLencode(paste0("otpauth://totp/", session$userData$user()$email, "?secret=", base_32_secret, "&issuer=", noclocksauthr$app_name))
        )
      )
    } else {
