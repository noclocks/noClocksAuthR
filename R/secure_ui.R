...
#' UI to take Admins from the custom Shiny app to the \code{noClocksAuthR} Admin Panel.  Set to
#' \code{NULL} to exclude the button.
#' @param admin_ui_options list of HTML elements to customize branding of the \code{noClocksAuthR} Admin Panel.  This
...
secure_ui <- function(
  ...
  function(request) {
    ...
    if (!is.null(cookie_string)) {
      noclocksauthr_cookie <- get_cookie(cookie_string, "noClocksAuthR")
      hashed_cookie <- digest::digest(noclocksauthr_cookie)
    }
      ...
      tryCatch({
        ...
        if (!is.null(user)) {
          ...
          if (is.na(user$signed_in_as)) {
            noclocksauthr_user <- user[
              c("session_uid", "user_uid", "email", "is_admin", "hashed_cookie", "email_verified", "roles", "two_fa_verified")
            ]

          } else {
            noclocksauthr_user <- get_signed_in_as_user(
              user_uid = user$signed_in_as
            )
            noclocksauthr_user$session_uid <- user$session_uid
            noclocksauthr_user$hashed_cookie <- user$hashed_cookie
            noclocksauthr_user$email_verified <- user$email_verified
            noclocksauthr_user$two_fa_verified <- user$two_fa_verified
          }
        ...
        if (isTRUE(noclocksauthr$is_auth_required) && identical(err$message, "user not invited") && !identical(page_query, "sign_in")) {
          force_sign_out <<- TRUE
        }
    ...
    if (isTRUE(force_sign_out)) {
      # send a random uuid as the noclocksauthr_session.  This will trigger a session
      # reload and a redirect to the sign in page
      return(tagList(
        tags$script(src = "noclocksauthr/js/router.js?version=4"),
        tags$script(src = "noclocksauthr/js/noclocksauthr_session.js?version=2"),
        tags$script(paste0("noclocksauthr_session('", uuid::UUIDgenerate(), "')"))
      ))
    }


    request$noclocksauthr_user <- noclocksauthr_user
    ...
    if (is.null(user)) {
      ...
      if (identical(page_query, "sign_in")) {
        ...
        if (is.null(sign_in_page_ui)) {
          ...
          page_out <- tagList(
            ...
            tags$script(src = "noclocksauthr/js/router.js?version=4"),
            tags$script(src = "noclocksauthr/js/noclocksauthr_session.js?version=2"),
            tags$script(paste0("noclocksauthr_session('sign_in-", uuid::UUIDgenerate(), "')"))
          )
        ...
        if (isFALSE(noclocksauthr$is_auth_required)) {
          ...
          page_out <- tagList(
            ...
            tags$script(src = "noclocksauthr/js/router.js?version=4"),
            tags$script(src = "noclocksauthr/js/noclocksauthr_session.js?version=2"),
            tags$script(paste0("noclocksauthr_session('", user$hashed_cookie, "')"))
          )
        } else {
          # send a random uuid as the noclocksauthr_session.  This will trigger a session
          # reload and a redirect to the sign in page
      ...
      if (identical(page_query, "sign_in")) {
        # send signed in session to noclocksauthr_session.  This will trigger
        ...
        page_out <- tagList(
          ...
          isFALSE(noclocksauthr$is_email_verification_required)) {

        if (isTRUE(noclocksauthr$is_two_fa_required) && isFALSE(user$two_fa_verified)) {
