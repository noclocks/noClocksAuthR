...
#' global configuration for `noClocksAuthR` authentication
#'
#' @details
#' This is the primary function for configuring \code{noClocksAuthR}.  It configures your app's instance of
#' the \code{noClocksAuthR} class that manages \code{noClocksAuthR} authentication.  Call this function in
#' your \code{global.R} file.
#' for a complete example.
#'
#' @param app_name the name of the Shiny app.
#' @param api_key the `noClocksAuthR` API key.
#' @param firebase_config if using Social Sign In, a list containing your
#' Firebase project configuration (Default: \code{NULL}).
...
noclocksauthr_config <- function(
  ...
  if (!(length(api_key) == 1 && is.character(api_key))) {
    stop("invalid `api_key` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }

  set_api_key(api_key)

  if (!((is.numeric(cookie_expires) && cookie_expires > 0) || is.null(cookie_expires))) {
    stop("invalid `cookie_expires` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }
...
call. = FALSE
  ...
  if (!(length(sign_in_providers) >= 1 && is.character(sign_in_providers))) {
    stop("invalid `sign_in_providers` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }

  if (!(length(is_invite_required) == 1 && is.logical(is_invite_required))) {
    stop("invalid `is_invite_required` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }
  if (!(length(is_email_verification_required) == 1 && is.logical(is_email_verification_required))) {
    stop("invalid `is_email_verification_required` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }
  if (!(length(is_two_fa_required) == 1 && is.logical(is_two_fa_required))) {
    stop("invalid `is_two_fa_required` argument passed to `noclocksauthr_config()`", call. = FALSE)
  }



  assign("app_name", app_name, envir = noclocksauthr)
  assign("app_uid", app$uid, envir = noclocksauthr)
  assign("api_key", api_key, envir = noclocksauthr)
  assign("firebase_config", firebase_config, envir = noclocksauthr)
  assign("is_invite_required", is_invite_required, envir = noclocksauthr)
  assign("sign_in_providers", sign_in_providers, envir = noclocksauthr)
  assign("is_email_verification_required", is_email_verification_required, envir = noclocksauthr)
  assign("cookie_expires", cookie_expires, envir = noclocksauthr)
  assign("is_auth_required", is_auth_required, envir = noclocksauthr)
  assign("is_two_fa_required", is_two_fa_required, envir = noclocksauthr)


  if (!is.null(firebase_config)) {
    if (length(firebase_config) != 3 ||
        !all(names(firebase_config) %in% c("apiKey", "authDomain", "projectId"))) {
      stop("invalid `firebase_config` argument passed to `noclocksauthr_config()`", call. = FALSE)
    }
...
#' @rdname noclocksauthr_config
#'
#' @param ... arguments to pass to \code{\link{noclocksauthr_config}}
#'
#' @export
#'
global_sessions_config <- function(
  ...
  .Deprecated("noclocksauthr_config")
