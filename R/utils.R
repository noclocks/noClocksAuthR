  ...
  if (!is.null(app_url) && !is.na(app_url) && app_url != "") {
    ...
    email_invite_checkbox <- tags$div(
      ...
      tags$span(
        ...
        `data-toggle` = "tooltip",
        `data-placement` = "top",
        title = "You must set the App URL to send email invites. Go to https://dashboard.noclocks.dev to set your app URL."
      )
    )
  }
...
noclocksauthr_toast_options <- list(
...
#' element if there is an error status code returned from the noClocksAuthR Auth API request.
#'
#' @noRd
#'
is_email_registered <- function(email) {
  ...
  user_res <- httr::GET(
    paste0(noclocksauthr$api_url, "/users"),
    query = list(
      email = email
    ),
    httr::authenticate(
