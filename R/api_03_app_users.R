#' noClocksAuthR API - Get App(s) User(s)
#'
...
#'
#' @return an object of class \code{noclocksauthr_api_res}.  When successful, the `content` of the object is a
...
get_app_users <- function(
  ...
  resp <- httr::GET(
    url = paste0(noclocksauthr$api_url, "/app-users"),
    ua,
    httr::authenticate(
      user = api_key,
      password = ""
  ...
  resp_out <- noclocksauthr_api_res(resp)
...
#' noClocksAuthR API - Add a User to an App
#'
#' @param app_uid the app uid.
#' @param user_uid an optional user uid for the user to be invited to the app.
#' @param email an optional user email address.
#' @param is_admin boolean (default: \code{FALSE}) - whether or not the user is a noClocksAuthR admin.
...
#'
#' @return an object of class \code{noclocksauthr_api_res}.  When successful, the `content` of the
#' \code{noclocksauthr_api_res} is \code{list(message = "success")}.  In the case of an error, the
...
add_app_user <- function(
  ...
  noclocksauthr_api_res(resp)
}




#' noClocksAuthR API - Update an App User
#'
...
#' noClocksAuthR API - Delete an App User
#'
