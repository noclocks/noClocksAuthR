...
test_app <- get_apps(app_name = "noclocksauthr_test")

test_app <- test_app$content

if (identical(nrow(test_app), 0L)) {
  add_app(app_name = "noclocksauthr_test")
...
# test_user <- get_users(email = "test1@noclocks.dev")
#
# test_user <- test_user$content
#
# if (identical(nrow(test_user), 0L)) {
#   add_user(email = "test1@noclocks.dev")
#
#   test_user <- get_users(email = "test1@noclocks.dev")
#
#   test_user <- test_user$content
